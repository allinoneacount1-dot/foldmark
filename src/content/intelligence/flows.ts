...
export const FLOWS_ENTRIES: Entry[] = [
...

  {
    ...
    answer:
      "DEX_BUY is assigned when the sending side of a transfer is an address the contracts registry holds as a dex_pool. Value left the pool and arrived at the receiver, which is what a purchase looks like from the pool's side.\n\nOnly that one fact produces the label. A token's name, the size of the amount, the number of transfers between the two addresses and any resemblance to trading activity are all irrelevant to it.\n\nWhere no address on an edge is held as a dex_pool, DEX_BUY is not assigned and the edge reads UNCLASSIFIED instead. On a deployment whose registry identifies no pool at all, that chip counts zero. That is the rule working, not a gap in it.",
    shortAnswer:
      "DEX_BUY means the sending side of the transfer is a registered DEX pool — value left the pool toward the receiver. Where the registry identifies no pool on the edge it is never assigned, and the edge reads UNCLASSIFIED.",
    detail:
